...
// ValueProposition removed from homepage — component file kept intact
  ...
  return (
    <section style={{ background: 'var(--off-white)', padding: '140px 32px', borderTop: '1px solid var(--border)' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 48px' }}>
        {/* Roadmap anchor for smooth scroll */}
...
function HomePage({ t, setPage }) {
  return (
    <>
      ...
      {/* ValueProposition removed — <ValueProposition t={t} /> */}
      <AboutSection t={t} />
      <Compliance t={t} />
    </>
  )
...
export default function App() {
  const [lang, setLang] = useState(() => {
    // 1. If localStorage has saved language → use that
    // 2. Else → default to 'ZH' (Simplified Chinese), NOT English
  ...
  // Defensive fallback: missing language key never blanks the app.
  ...
  // Standard router owns history; this effect handles route-change scroll only.
  useEffect(() => {
    window.scrollTo(0, 0)
  }, [location])

  // Set the html lang attribute for screen readers / browsers.
  useEffect(() => {
    const map = { EN: 'en', ZH: 'zh-CN', TW: 'zh-TW' }
    document.documentElement.lang = map[lang] || 'zh-CN'
  }, [lang])

  // Persist language selection permanently
  useEffect(() => {
    localStorage.setItem('nexallure_lang', lang)
  }, [lang])

  // Check for remembered device on app load
  useEffect(() => {
    ...
    <ErrorBoundary>
      <AuthProvider>
        {!["/register", "/login", "/privacy", "/terms"].includes(location) && (
          <Preloader lang={lang} heroClassName="bg-[#0A0F1E]" />
        )}
